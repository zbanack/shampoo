...
function shampoo_var_catch(canvas) {
	...
	switch(canvas.uid) {
		case("main"):
			global.player_name = canvas.textarea_get_value("player_name");			score = canvas.slider_get_value("player_score");
		break;
	}
...
```[textbox, value="This value~, right here~, has commas" /][button, label="Hello~, World!", bg=blue, radius=5, c=white /]## Spaces and Linebreaks
